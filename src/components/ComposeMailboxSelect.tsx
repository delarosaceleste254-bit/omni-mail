...
export function ComposeMailboxSelect({
  mailboxes,
  value,
  disabled,
  onChange,
  ...
  const menuId = useId()
  const selectedIndex = Math.max(0, mailboxes.findIndex((mailbox) => mailbox.address === value))
  ...
  return (
    <div className={`compose-mailbox-select${open ? ' is-open' : ''}`} ref={root}>
      <button
        ref={trigger}
      ...
      >
        <span className="compose-mailbox-select__icon"><AtSign size={14} /></span>
        <span>{selected?.address || value}</span>
        <ChevronDown size={15} aria-hidden="true" />
      </button>
      {open && (
        <div
          className="compose-mailbox-select__menu"
          id={menuId}
          role="listbox"
          aria-label={t('发件人')}
        ...
        >
          {mailboxes.map((mailbox, index) => (
            <button
              ...
              className={mailbox.address === value ? 'is-selected' : ''}
              type="button"
              role="option"
              aria-selected={mailbox.address === value}
              tabIndex={mailbox.address === value ? 0 : -1}
              key={mailbox.address}
              onClick={() => {
                onChange(mailbox.address)
                closeMenu(true)
              }}
            >
              <span className="compose-mailbox-select__icon"><AtSign size={14} /></span>
              <span>
                ...
                <small>{mailbox.domain}{mailbox.isPrimary ? ` · ${t('主邮箱')}` : ''}</small>
              </span>
              {mailbox.address === value && <Check size={15} />}
            </button>
          ))}
        </div>
      )}
